...
public class SaveLoad {
	...
	public SaveLoad() {
		dir = new File(Minecraft.getMinecraft().mcDataDir,"Main");
		if(!dir.exists()) {
			dir.mkdir();
		}
	...
	public void save() {
		...
		for (Module mod : ExampleMod.instance.moduleManager.modules) {
			toSave.add("MOD: " + mod.getName() + ":" + mod.isToggled() + ":" + mod.getKey());
		}
		for (Setting set : ExampleMod.instance.settingsManager.getSettings()) {
			if (set.isCheck()){
	...
	public void load() {
		...
		for (String s : lines) {
			...
			if(s.toLowerCase().startsWith("mod:")) {
				Module m = ExampleMod.instance.moduleManager.getModule(args[1]);
				if(m!=null) {
					m.setToggled(Boolean.parseBoolean(args[2]));
					m.setKey(Integer.parseInt(args[3]));
				}
			}else if (s.toLowerCase().startsWith("set:")) {
				Module m = ExampleMod.instance.moduleManager.getModule(args[2]);
				if(m!=null) {
					Setting set = ExampleMod.instance.settingsManager.getSettingByName(m, args[1]);
					if(set!=null) {
						if(set.isCheck()) {
